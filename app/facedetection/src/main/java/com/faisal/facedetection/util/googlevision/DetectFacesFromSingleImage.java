...
import com.faisal.facedetection.R;

import com.faisal.facedetection.util.Constants;
import com.faisal.facedetection.util.FaceMathUtils;
import com.faisal.facedetection.util.GetContext;
import com.faisal.facedetection.util.SharedPrefUtil;
import com.faisal.facedetection.util.ValidationObject;
        ...
        try {
            ...
            if (faces != null) {
                ...
                if (faces.size() == 0) {
                    ...
                    validationObject.setMessage(context.getResources().getString(R.string.no_faces_found));
                    ...
                    if (point1.x != 0 && point6.x != 0 && point7.x != 0) {
                        ...
                        if (valueComparison > Constants.VALUE_COMPARISON_MINIMUM_THRESOLD) {
                            validationObject.setSuccess(false);
                            validationObject.setMessage(context.getResources().getString(R.string.error_face_position));
                        } else if (distMid_6 > (Constants.MINIMUM_DISTANCE_OFFSET - 130)) {
                            validationObject.setSuccess(false);
                            validationObject.setMessage(
                                    context.getResources().getString(R.string.error_face_center));
                            ...
                            validationObject.setSuccess(false);
                            validationObject.setMessage(context.getResources().getString(R.string.error_eyes_closed));
                        } else if (face.getIsLeftEyeOpenProbability() <= Constants.EyeOpenThresold) {
                            validationObject.setSuccess(false);
                            validationObject.setMessage(context.getResources().getString(R.string.error_left_eyes_closed));
                        } else if (face.getIsRightEyeOpenProbability() <= 0.8) {
                            validationObject.setSuccess(false);
                            validationObject.setMessage(context.getResources().getString(R.string.error_right_eyes_closed));
                        }
                    }
                    ...
                    //multi face detected
                    validationObject.setSuccess(false);
                    validationObject.setMessage(context.getResources().getString(R.string.more_image));
